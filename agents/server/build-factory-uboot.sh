...
trap 'sed -i "s/^# CONFIG_ANTIROLLBACK is not set$/CONFIG_ANTIROLLBACK=y/" "$CFG"' EXIT
sed -i 's/^CONFIG_ANTIROLLBACK=y$/# CONFIG_ANTIROLLBACK is not set/' "$CFG"
...
sed -i 's/^# CONFIG_ANTIROLLBACK is not set$/CONFIG_ANTIROLLBACK=y/' "$CFG"
